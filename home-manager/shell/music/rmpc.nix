...
in
{
  #TODO: rmpc not reading from config due to unknown error.
  config = mkIf config.custom.rmpc.enable {
    home = {
      ...
      #         (
      #             on_song_change: ["${getExe config.custom.mpdSongChange.package}"]
    ...
    # custom.persist.home.directories = [ ".config/rmpc" ];
  };
}
